...
ttt<-function(boardstate=as.character(c(1:9))){structure(boardstate,class="ttt")}
...
library('dplyr')
vehicles %>% summarise(num_of_makers=n_distinct(make))
```

There are 128 unique vehicle makers.
...
# write your code here, the output displayed should answer the question.
#View(vehicles)
vehicles %>% filter(year=="2014") %>% summarise(n=n())
```

There are 1214 vehicles
...
```{r}
# write your code here, the output displayed should answer the question.
vehicles %>% filter(class=="Compact Cars" & year==2014) %>% summarise(mean(cty))
vehicles %>% filter(class=="Midsize Cars" & year==2014) %>% summarise(mean(cty))
```

The average city mpg for 23.9204 for compact car and 23.51174 for midsize car.
...
```{r}
# write your code here, the output displayed should answer the question.
d<-vehicles %>% group_by(make) %>% filter(class=="Midsize Cars" , year==2014) %>% summarise(mean_cty=mean(cty)) %>% arrange(desc(mean_cty))
print.data.frame(d)
...
```{r}
# load("dr4.Rdata")
load("/Users/lingjuexie/Downloads/dr4.Rdata")
#View(dr4)
#View(twice)
#View(three)
#View(four)
#View(five)
twice<-dr4 %>% filter((!is.na(visit2)) & is.na(visit3)) %>% mutate(diffdate = difftime(visit2,visit1,units="days")) %>% summarise(totaldif=sum(diffdate),count=n())

three<-dr4 %>% filter((!is.na(visit3)) & is.na(visit4)) %>% mutate(diffdate = difftime(visit3,visit1,units="days")) %>% summarise(totaldif=sum(diffdate),count=n())

four<-dr4 %>% filter((!is.na(visit4)) & is.na(visit5)) %>% mutate(diffdate = difftime(visit4,visit1,units="days")) %>% summarise(totaldif=sum(diffdate),count=n())

five<-dr4 %>% filter(!is.na(visit5)) %>% mutate(diffdate = difftime(visit5,visit1,units="days")) %>% summarise(totaldif=sum(diffdate),count=n())

(twice$totaldif + three$totaldif + four$totaldif + five$totaldif)/(twice$count+three$count*2+four$count*3+five$count*4)
...
teamname <- teampage %>%html_nodes("#teams_active .left a") %>%html_text()
...
for(i in teamname[1:30]){
  activeteam[[i]] <- s %>% follow_link(i) %>% read_html()%>% html_nodes("#franchise_years") %>% html_table() %>% as.data.frame() 
  
  activeteam[[i]]$current<-i
}

baseball<-rbindlist(activeteam)
...
baseball2<-baseball %>% filter(Year>=2001 & Year<=2016) %>% group_by(current) %>% summarise(wins=sum(W),losses=sum(L),runs=sum(R),runsAllowed=sum(RA)) %>% mutate(win_pre=wins/(wins+losses)) %>% arrange(desc(win_pre))
print.data.frame(baseball2)
```
...
manager <- baseball %>% select(Managers)
#View(manager)
...
for( i in 1:nrow(manager)){
  ...
  name<-str_match_all(manager[i],"([A-Z].*?)\\s\\(")
  wins<-str_match_all(manager[i],"\\(([0-9]{1,})\\-")
  loses<-str_match_all(manager[i],"\\-([0-9]{1,})\\)")
  for(j in 1:nrow(name[[1]])){
    df<-rbind(df,data.frame(name[[1]][j,2],as.numeric(wins[[1]][j,2]),as.numeric(loses[[1]][j,2])))
  }
}
 
#View(df)
colnames(df)<-c("manager","w","l")
managerT = df %>% group_by(manager) %>% summarise(wins=sum(w), losses=sum(l)) %>% mutate(games=wins+losses,winpercentage=wins/games) %>% arrange(desc(games)) %>% select(manager,games,wins,losses,winpercentage)
